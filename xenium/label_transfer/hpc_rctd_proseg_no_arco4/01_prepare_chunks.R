...
## Xenium data (all ten sections), identical to
## xenium/label_transfer/hpc_rctd_proseg/ EXCEPT the reference additionally
## excludes Glut-Arco-4 (below, on top of the existing Glut-Nido-3 exclusion).
## See that directory's 01_prepare_chunks.R for the full rationale on sizing,
## the fixed-vs-scaling cost split, and the proseg gene-order gotcha — all
## unchanged here. Only EXCLUDE and the output/staging paths differ.
##
...
OUT <- if (length(args) > 0) args[1] else
  file.path(path.expand(XENIUM_PROC_DIR), "hpc_rctd_proseg_no_arco4")
CHUNK <- 2e4
UMI_MIN <- 20
SETUP_CORES <- 4
MAX_REF_PER_LABEL <- 1000
EXCLUDE <- c("Glut-Nido-3", "Glut-Arco-4")
ASSAY_REF <- "RAW"
...
## ---- reference (same config as hpc_rctd_proseg, minus Glut-Arco-4) -------
...
message("reference: ", ncol(counts_ref), " cells, ", nlevels(labels_rctd), " labels, ",
        length(ps), " genes")
message("excluded: ", paste(EXCLUDE, collapse = ", "))

## ---- query: load each proseg section, re-index genes by NAME ------------
## gene_lists/ is duplicated locally here (same 10 files as hpc_rctd_proseg —
## proseg wasn't re-run) so this directory stays self-contained.
gene_list_dir <- here::here("xenium/label_transfer/hpc_rctd_proseg_no_arco4/gene_lists")
...
script_dir <- here::here("xenium/label_transfer/hpc_rctd_proseg_no_arco4/scripts")
...
## this pipeline's proseg sibling (79 actual chunks vs. 80 carried over from
## the delivered-segmentation run) — a guaranteed one-task failure per
## submission, masked at the time by an unrelated fatal error (stale
## BASH_SOURCE path) that failed every task first regardless. Patching the
## copy under OUT means what actually gets rclone'd to the cluster is always
## correct, independent of what value is checked into the repo template.
