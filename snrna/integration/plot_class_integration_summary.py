...
# Type/colour follow config/figure_theme.R (Arial, 5 pt tick labels, 6 pt axis titles,
# FIG_PAL slots 1-3 in fixed order, FIG_INK_* / FIG_GRID / FIG_AXIS tokens).
...
COLORS = {"Non-neuronal": "#2a78d6", "GABAergic": "#eb6834", "Glutamatergic": "#1baf7a"}  # FIG_PAL[1:3]
...
fig, axes = plt.subplots(2, 2, figsize=(3.6, 3.3), sharey="row")
    ...
    for i, (met, ylabel, ylim) in enumerate(METRICS):
            ...
            if met == "agree_frac":   # discrete 0.25 steps: fan out horizontally, wrap rows of 6
                for val in np.unique(v):
                    ...
                    for r0 in range(0, n, 6):
                        ...
                        ax.scatter(xs, ys, s=7, color=COLORS[c], edgecolor="white",
                                   linewidth=0.4, zorder=4)
            else:
                xs = k + rng.uniform(-0.18, 0.18, len(v))
                ax.scatter(xs, v, s=7, color=COLORS[c], edgecolor="white", linewidth=0.4, zorder=4)
        ...
        for k, (a, b) in enumerate([("Non-neuronal", "GABAergic"), ("GABAergic", "Glutamatergic")]):
            ...
            ax.plot([k + 0.05, k + 0.05, k + 0.95, k + 0.95], [y - 0.02, y, y, y - 0.02],
                    color=INK, linewidth=0.5, clip_on=False)
            ax.text(k + 0.5, y + 0.01, pstar(p), ha="center", va="bottom", fontsize=5.5, color=INK)
        ax.set_ylim(0, 1.0)
        ...
        ax.tick_params(length=2, width=0.5, color=AXIS, labelcolor=INK, labelsize=5)
        ax.grid(axis="y", color=GRID, linewidth=0.4, zorder=0)
        ax.set_axisbelow(True)
        if met == "confidence":
            ax.axhline(0.5, color=MUTED, linewidth=0.5, linestyle=(0, (3, 2)), zorder=2)
            if j == 1:
                ax.text(2.55, 0.5, " high\n tier", va="center", ha="left", fontsize=4.5, color=MUTED)
        if j == 0:
            ax.set_ylabel(ylabel, fontsize=6)
        if i == 0:
            ax.set_title(label, fontsize=6.5, pad=13, color=INK)
...
fig.text(0.5, 0.01,
         ...
         ha="center", va="bottom", fontsize=4.5, color=MUTED)
fig.subplots_adjust(left=0.17, right=0.95, top=0.9, bottom=0.14, hspace=0.6, wspace=0.12)
for ext in ("pdf", "png"):
    fig.savefig(OUT / f"class_integration_summary.{ext}", dpi=600)
print(f"\nwrote {OUT}")
