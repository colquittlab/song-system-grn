"""SATURN input CSV for the hybrid-labeled Glut(finch)-only x Excitatory(chicken)-only
subset. Same transposition-margin guard as make_run_csv.py; only the finch h5ad path
changes (bf_adult_glut_hybrid.h5ad). Chicken side is untouched.
"""
...
ROWS = [
    ("finch", DATA / "bf_adult_glut_hybrid.h5ad", E / "GCF_005870125.1_lonStrDom2_translated_cds.gene_symbol_to_embedding_ESM1b.pt"),
...
if __name__ == "__main__":
    ...
    out = DATA / "bf_gg_glutonly_hybrid_run.csv"
    df.to_csv(out, index=False)
    print(f"\nwrote {out}")
    clusters = pd.concat([pd.DataFrame({"species": s, "cluster": sorted(ad.read_h5ad(h, backed="r").obs["cluster"].unique())})
                          for s, h, _ in ROWS])
    clusters.to_csv(DATA / "bf_gg_glutonly_hybrid_clusters.csv", index=False)
    print(f"wrote {DATA / 'bf_gg_glutonly_hybrid_clusters.csv'} ({len(clusters)} labels)")
