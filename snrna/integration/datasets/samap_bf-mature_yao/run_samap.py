...
BASE = Path("/private/groups/colquittlab/song-system-grn/snrna/integration/datasets/samap_bf-mature_yao")
LM = Path("/private/groups/colquittlab/saturn/snrna-bf-dev_snrna-lamanno-dev/data")
YAO = Path("/private/groups/colquittlab/saturn/snrna-bf-dev_snrna-yao2023/data")
...
def main(out_dir: Path, numiters: int, ncpus: int, seed: int, finch_h5ad: str):
    out_dir.mkdir(parents=True, exist_ok=True)
    sams = {
        "bf": load_sam(Path(finch_h5ad), "cluster", "finch", True),
        "mm": load_sam(YAO / "mm_yao.h5ad", "cluster", "mouse", True),
    }
    keys = {"bf": "cluster", "mm": "cluster"}

    sm = SAMAP(sams, f_maps="/private/groups/colquittlab/saturn/samap_bf-dev_lamanno/maps_full/", keys=keys)
    print("\nrunning SAMap...")
    sm.run(NUMITERS=numiters, ncpus=ncpus, umap=True)
    ...
    mm_cols = [c for c in MT.columns if str(c).startswith("mm_")]
...
if __name__ == "__main__":
    ...
    p.add_argument("--finch_h5ad", type=str, default=str(LM / "bf_dev_v3_mature.h5ad"))
