"""SATURN input CSV for the hybrid-labeled adult finch x adult chicken pair.

Same transposition-margin guard as make_run_csv.py; only the finch h5ad path changes
(bf_adult_hybrid.h5ad). Chicken side is untouched.
"""
...
ROWS = [
    ("finch", DATA / "bf_adult_hybrid.h5ad", E / "GCF_005870125.1_lonStrDom2_translated_cds.gene_symbol_to_embedding_ESM1b.pt"),
...
if __name__ == "__main__":
    ...
    out = DATA / "bf_gg_adult_hybrid_run.csv"
    ...
    clusters = pd.concat([
        ...
        for s, h, _ in ROWS])
    clusters.to_csv(DATA / "bf_gg_adult_hybrid_clusters.csv", index=False)
    print(f"wrote {DATA / 'bf_gg_adult_hybrid_clusters.csv'} ({len(clusters)} labels)")
